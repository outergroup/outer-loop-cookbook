...
def plot(model_name):
    ...
    for compile in [True, False]:
        ...
        for vectorize in [True, False]:
            ...
            for num_models in all_num_models_values:
                filename = f"{results_dir}/mnist1-{model_name}-benchmark_covariance_fit-repetitions30-compile{compile_str}-vectorize{vectorize_str}-num_models{num_models}.csv"
    ...
    filename = os.path.join("images", f"benchmark_fit_{model_name}.svg")
    print(f"Saving figure to {filename}")
    plt.savefig(filename)


plot("VexprHandsOnGP")
plot("VexprPartialHandsOnGP")
